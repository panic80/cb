...
import React, { useState, useEffect } from 'react'; // Keep useEffect for initial scroll if needed elsewhere, but can remove here
...
import { unitContacts, allUnits, fscContacts, fmcContacts } from '../data/opiContacts';
...
export default function OPIPage({ theme, onThemeChange }) {
  ...
  const renderContactList = (contacts, title = null, isLeadership = false) => (
    <div className="card-hover glass p-4 sm:p-6 rounded-lg border border-[var(--border-color)]">
      {title && <h3 className="font-semibold mb-4 text-lg sm:text-xl text-[var(--primary)] border-b border-[var(--border-color)] pb-2">{title}</h3>}
      <ul className="space-y-4">
        {contacts.map((contact, index) => (
          <li key={`${contact.email}-${index}`}> {/* Use email + index for a more robust key */}
            <ContactCard
              name={contact.name}
              email={contact.email}
              units={contact.units}
     ...
     <div className="space-y-8">
        <div className="grid gap-6 md:grid-cols-2">
            {groups.map((group, index) => (
                <div key={`group-${index}`} className="card-hover glass p-4 sm:p-6 rounded-lg border border-[var(--border-color)]">
        ...
        {additionalContacts && additionalContacts.length > 0 && (
            <div className="card-hover glass p-4 sm:p-6 rounded-lg border border-[var(--border-color)]">
                <h3 className="font-semibold mb-4 text-lg sm:text-xl text-[var(--primary)] border-b border-[var(--border-color)] pb-2">Additional Contacts</h3>
  ...
  return (
    <PageLayout title="OPI Contact Information" theme={theme} onThemeChange={onThemeChange} useGradientBackground={true}>
      ...
      <> {/* Main content fragment */}
        <div className="text-center mb-8 sm:mb-12 transition-all duration-300">
          ...
          <h1 className="text-2xl sm:text-4xl font-bold mb-2 sm:mb-3 gradient-text">Office of Primary Interest</h1>
          <p className="text-base sm:text-lg text-[var(--text-secondary)] transition-colors duration-300 px-4 max-w-2xl mx-auto glass p-4 rounded-xl">
            Find your unit's point of contact for financial services and management
          </p>
        </div>
          ...
          <div className="p-4 sm:p-6 border-b border-[var(--border-color)]">
            <div className="flex flex-col sm:flex-row gap-2 sm:gap-4 justify-center">
              {['fsc', 'fmc', 'byUnit'].map((view) => (
                ...
                >
                  {view === 'fsc' ? 'Financial Services (FSC)' : view === 'fmc' ? 'Financial Management (FMC)' : 'Find by Unit'}
                </button>
              ))}
            </div>
          </div>
            ...
            {contactView === 'byUnit' ? (
              <div className="space-y-6 sm:space-y-8">
                ...
                <div className="glass p-4 sm:p-6 rounded-lg border border-[var(--border-color)]">
                  <label htmlFor="unit-select" className="block font-semibold mb-3 text-lg text-[var(--text-primary)]">Select Your Unit</label>
                ...
                {selectedUnit && unitContacts[selectedUnit] ? (
                  <div className="grid gap-6 md:grid-cols-2">
                    {/* FSC Card */}
                    <div className="card-hover glass p-4 sm:p-6 rounded-lg border border-[var(--border-color)]">
                      <h3 className="font-semibold mb-4 text-lg sm:text-xl text-[var(--primary)] border-b border-[var(--border-color)] pb-2">Your FSC Contact</h3>
                      {unitContacts[selectedUnit].fsc !== 'N/A' ? (
                        ...
                        />
                         ...
                         <div className="text-[var(--text-secondary)] p-4 bg-[var(--bg-primary)] rounded-lg border border-dashed border-[var(--warning-color)]"> {/* Warning style */}
                            <p className="font-medium">No direct FSC assigned.</p>
                            <p className="text-sm">Please contact FSC Leadership listed under the "Financial Services (FSC)" tab.</p>
                         </div>
                      )}
                    </div>
                    ...
                    {/* FMC Card */}
                    <div className="card-hover glass p-4 sm:p-6 rounded-lg border border-[var(--border-color)]">
                      <h3 className="font-semibold mb-4 text-lg sm:text-xl text-[var(--primary)] border-b border-[var(--border-color)] pb-2">Your FMC Contact</h3>
                       <ContactCard
                          name={unitContacts[selectedUnit].fmc}
                          email={unitContacts[selectedUnit].fmcEmail}
                          title="Financial Management Cell (FMC)"
                        />
                    </div> {/* Closing FMC Card div */}
                  {/* Closing grid div from line 137 */} </div>
                ) : (
                   <div className="glass text-center py-8 px-4 bg-[var(--bg-primary)] rounded-lg border border-[var(--border-color)]"> {/* Removed dashed */}
                       <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-10 h-10 mx-auto text-[var(--text-tertiary)] mb-3">
                          <path strokeLinecap="round" strokeLinejoin="round" d="M15 9h3.75M15 12h3.75M15 15h3.75M4.5 19.5h15a2.25 2.25 0 002.25-2.25V6.75A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25v10.5A2.25 2.25 0 004.5 19.5zm6-10.125a1.875 1.875 0 11-3.75 0 1.875 1.875 0 013.75 0zm1.294 6.336a6.721 6.721 0 01-3.17.789 6.721 6.721 0 01-3.168-.789 3.376 3.376 0 016.338 0z" />
                        </svg>
                      <p className="text-[var(--text-secondary)] font-medium">Please select your unit above.</p>
                      <p className="text-sm text-[var(--text-tertiary)]">Your specific FSC and FMC contacts will be displayed here.</p>
                   </div>
                )}
              </div>
            ) : contactView === 'fsc' ? (
                 <div className="space-y-8">
                    {renderContactList(fscContacts.leadership, 'FSC Leadership', true)}
                    <div className="grid gap-6 md:grid-cols-2">
                        {fscContacts.sections.map((section, index) => (
                            <div key={`fsc-sec-${index}`} className="bg-[var(--bg-primary)] p-4 sm:p-6 rounded-lg border border-[var(--border-color)]"> {/* Added border */}
                                <h3 className="font-semibold mb-4 text-lg sm:text-xl text-[var(--primary-color)] border-b border-[var(--border-color)] pb-2">{`Section ${index + 1}`}</h3> {/* Enhanced title */}
                                <ContactCard
                                    name={section.name}
                                    email={section.email}
                                    units={section.units}
                 ...
                 <div className="space-y-8">
                    {fmcContacts.leadership && renderContactList(fmcContacts.leadership, 'FMC Leadership', true)}
                    {renderGroupedContacts(fmcContacts.groups, fmcContacts.additional)}
                 </div>
            )}
          </div>
        </div>
      </> {/* Closing main content fragment */}
      </div> {/* Closing hero-gradient div */}
    </PageLayout>
  );
}